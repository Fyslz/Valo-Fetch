...
if response.status_code == 200: # make sure response is 200
    ...
    while 1:
        wanted_agent = (input("Select the Agent (to select all type ALL) : ")).lower()
        if wanted_agent == "all":
            break
        elif wanted_agent in [a.lower() for a in valorant_agents]:
            chosen_agent = [a for a in valorant_agents if a.lower() == wanted_agent][0]
            valorant_agents = [chosen_agent]
            break
        else:
    ...
    for i in range(len(agents_data)):
        for j in range(len(keys_to_check)):
            if keys_to_check[j] == "role":
                agent[agents_data[i]["displayName"]][f"role_icon"] = agents_data[i][keys_to_check[j]]["displayIcon"]
                ...
                for k in range(len(agents_data[i][keys_to_check[j]])):
                    ...
                    agent[agents_data[i]["displayName"]][key] = value
    ...
    base_path = r"C:\Users\na3cl\OneDrive\Documents\valo_agents"
    ...
    for m in range(len(valorant_agents)):
        for n in range(len(agent[valorant_agents[m]])):
            ...
            if img_response.status_code == 200:
                ...
                print(f"Download Failure, error code: {img_response.status_code}")
